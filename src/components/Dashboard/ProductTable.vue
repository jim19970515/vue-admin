<script>
    ...
    export default{
        ...
        methods:{
            async getProducts(){
                const vm = this
                const {data} = await axios.get(`${vm.apiUrl}/api/${vm.apiPath}/admin/products/all`)
...
<template>
    <div class="col-span-2 bg-white rounded-10 p-4">
        ...
        <div class="relative shadow-md rounded-lg overflow-auto h-[calc(816px-184px-48px-32px-40px-12px)]">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                <thead class="text-xs uppercase bg-primary text-white sticky top-0">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 whitespace-nowrap">餐點名稱/內容/售價</th>
                        <th scope="col" class="px-6 py-3 whitespace-nowrap">是否啟用</th>
                    </tr>
                </thead>
                <tbody class="overflow-y-auto max-h-auto">
                    <tr v-for="(item,i) in products" :key="i" class="bg-gray-100 text-gray-900">
                        <th scope="row" class="px-6 py-4 font-medium  whitespace-nowrap">{{ i+1 }}</th>
                                <td class="px-6 py-4">
                                    <img :src="item.imageUrl" class=" h-20 w-20 object-cover rounded-10" alt="">
                                </td>
                                <td class="px-6 ">
                                    <h3 class="text-xl font-bold">{{item.title}}</h3>
                                    <p class="text-md font-normal text-gray-600">{{ item.content }}</p>
